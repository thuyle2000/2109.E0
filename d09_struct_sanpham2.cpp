...
Moi san pham bao gom cac thuoc tinh: ma so, ten, don gia, xuat xu
	...
	3. in ds san pham theo thu tu tang dan cua cot don gia
*/
...
int main() {
	...
	// Buoc 6: in danh sach san pham theo thu tu tang dan cua cot don gia 
	
	// Buoc 6a: sap xep lai ds theo thu tu tang dan cua cot don gia
	struct SANPHAM temp;
	for(int i=0; i<n-1; i++){
		for(int k=i+1; k<n; k++){
	...
	// Buoc 6b: in danh sach san pham da duoc sap xep theo thu tu tang dan cua cot don gia 
	printf("\n\n * * * Danh sach san pham theo thu tu tang dan cot don gia * * * \n");
	for(int i=0; i<n; i++) {
		printf("%-10s ", ds[i].ma_so);
		printf("%-20s ", ds[i].ten);
		printf("%6d ", ds[i].don_gia);
