...
Moi san pham bao gom cac thuoc tinh: ma so, ten, don gia, xuat xu
	...
	3. tim kiem san pham theo don gia
*/
...
int main() {
	...
	// Buoc 6: in danh sach san pham theo don gia dc yeu cau
	int dg;
	printf("\n\n >> nhap don gia san pham muon tim: ");
	scanf("%d" , &dg);

	int cnt=0;
	printf("\n\n * * * Danh sach san pham co don gia <= %d * * * \n", dg);
	for(int i=0; i<n; i++) {
		if(ds[i].don_gia <= dg) {
			printf("%-10s ", ds[i].ma_so);
			printf("%-20s ", ds[i].ten);
	...
	if(cnt == 0){
		printf(" >> KO TIM THAY SAN PHAM CO DON GIA <= %d !!! \n", dg);
	}
}
